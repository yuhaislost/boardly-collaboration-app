...
export const get = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.favourites)
        {
            const userFavourites = await ctx.db.query("userFavourites").withIndex("by_user_org", (q) => q.eq("userId", identity.subject).eq("orgId", args.orgId)).order('desc').collect();

            const boardIds = userFavourites.map((favourite) => favourite.boardId);
